...
class SwinTransition(nn.Module):
    def __init__(self, label_hierarchy, hid_dim=600, pretrained=False, fine_to_coarse=False, sprite_dim=2):
        ...
        # self.extractor = swin_t(weights=Swin_T_Weights.IMAGENET1K_V1)
        # self.extractor = swin_s(weights=Swin_S_Weights.IMAGENET1K_V1)
        self.extractor = swin_b(weights=Swin_B_Weights.IMAGENET1K_V1)
        self.feature_dim = self.extractor.head.in_features
...
if __name__ == "__main__":
    ...
    model = SwinTransition(label_hierarchy, pretrained=False, fine_to_coarse=True)
